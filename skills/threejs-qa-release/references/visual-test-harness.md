...
Inspect for frozen rigs, collapsing or stretching limbs, foot sliding relative to world displacement, root-motion double application, snapping transitions, looping attacks, and hit/contact events that disagree with the visible motion. Note clip names, durations, mixer action changes, and event times alongside observed defects. For FBX/store-kit bodies, a bind-pose still with clips on disk is a fail — confirm the catalog from `FbxActor.debugState()` / `template.animations` and the playbook in `threejs-gameplay-systems/references/fbx-animation.md`. Test active movement and interruption through real input, not only a forced pose. Rigid-body-only games need checks of their actual physics/motion, not an invented skeleton audit.
